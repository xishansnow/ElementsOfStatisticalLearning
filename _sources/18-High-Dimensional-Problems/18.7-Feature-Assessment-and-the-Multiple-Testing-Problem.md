...
多出来的“正的”表示我们只关心在有发现的条件下估计误差率。这是 FDR 的修改版本，它会有一个很简洁的贝叶斯表示。注意到通常的 FDR（式 式（ 18.43 ））当 $\mathrm{Pr}(R=0)>0$ 时没有定义。
...
因此在混合模型 式（ 18.51 ） 的假设下，如果测试统计量落在该测试的拒绝域中，也就是，如果我们拒绝原假设时，则 pFDR 是零假设为真的后验概率.（[练习 18.20](https://github.com/szcf-weiya/ESL-CN/issues/135)）。
