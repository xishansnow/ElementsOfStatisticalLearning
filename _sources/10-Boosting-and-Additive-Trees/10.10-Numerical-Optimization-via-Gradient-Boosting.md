...
$$
    ...
    &=-\sum\limits_{k=1}^KI(y={\mathbb{G}_k})f_k(x)+\log \left(\sum\limits_{\ell=1}^Ke^{f_\ell(x)}\right)\tag{10.22}
    \end{align*}
...
&=I(y_i={\mathbb{G}_k})-p_k(x_i)\tag{10.38}
\end{align*}
$$

其中 $p_k(x)$ 由 式（ 10.21 ） 给出。尽管在每次迭代时，分别构造 $K$ 棵树，但是它们通过式（ 10.21 ） 关联起来。
