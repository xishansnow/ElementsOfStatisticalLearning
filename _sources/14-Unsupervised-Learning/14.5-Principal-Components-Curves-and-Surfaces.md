...
$$
    ...
    w_i^\mathrm{\text{subject to}}ar(t) = w_i[h_i(t)]
...
$$
    ...
    w_i^\mathrm{\text{subject to}}ar(t_{j,\text{landmark}}) \approx w_j(t_{j,\text{landmark}})\,.
...
公式（ 14.64 ） 表明如果有较大邻接 $w_{ii'}$ 的数据对的坐标 $f_i$ 和 $f_{i'}$ 很接近，则 $\mathbf f^T\mathbf L\mathbf f$ 将达到较小的值。
