...
在图 3.7 中我们看到交互验证表明有 7 项；最终模型在表 3.3 中有最低的测试误差。
...
在前列腺癌的例子中，交互验证在图 3.7 中选择 $M=2$ 个 PLS 方向。这得到了表 3.3 最右边的列的模型。
...
\mathrm{\text{subject to}} &\Vert \alpha\Vert=1,\alpha^T\mathbf Sv_\ell=0,\;\ell=1,\ldots,m-1
\end{align*}
\tag{3.63}
$$
...
\mathrm{\text{subject to}}& \Vert\alpha\Vert=1,\alpha^T\mathbf S\hat \varphi_\ell=0,\ell=1,\ldots,m-1
\end{align*}
\tag{3.64}
$$
