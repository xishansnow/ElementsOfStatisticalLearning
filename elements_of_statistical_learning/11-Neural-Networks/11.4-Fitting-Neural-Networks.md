...
以及对应的分类器 $G(x)=\mathrm{arg\; max}_kf_k(x)$。有了 softmax 激活函数和交互熵误差函数，神经网络模型实际上是关于隐藏层的线性逻辑斯蒂回归模型，而且所有的参数通过极大似然来估计。
...
这个 two-pass 过程称作向后传播。也被称为 delta rule (Widrow and Hoff, 1960[^1])。交互熵计算的组分与平方误差函数和有同样的形式，并且在[练习 11.3]() 中被导出来。
