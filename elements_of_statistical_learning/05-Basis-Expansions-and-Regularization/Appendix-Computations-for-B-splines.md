...
$$
\hat\gamma = (\B^T\B+\lambda\oomega_B)^{-1}\B^Ty\tag{5.79}
$$
...
因此，矩阵 $\M=(\B^T\B)+\lambda\oomega$ 是 4-banded，则可以很简单地计算它的 cholesky 分解 $\M=\L\L^T$。接着，通过向后替换(back-substitution)求解 $\L\L^T\gamma=\B^Ty$ 得到 $\gamma$，则解 $\hat f$ 是 $O(N)$ 次运算量。
