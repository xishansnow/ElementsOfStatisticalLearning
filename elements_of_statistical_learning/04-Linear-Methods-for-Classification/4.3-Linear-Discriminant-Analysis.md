...
其中，$\hat\Sigma$ 是和 LDA 一样用的联合协方差矩阵。这里 $\alpha\in[0,1]$ 允许在 LDA 和 QDA 之间连续变化的模型，而且需要指定。实际中，$\alpha$ 可以基于在验证数据的表现上进行选择，或者通过交互验证。
