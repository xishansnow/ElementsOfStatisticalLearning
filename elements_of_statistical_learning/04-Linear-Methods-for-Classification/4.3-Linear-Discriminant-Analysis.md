...
作为下一主题的铺垫，我们简单地岔开去讨论 LDA，特别是 QDA 的计算。这些计算可以通过对角化 $\hat\ssigma$ 或 $\hat\ssigma_k$ 来简化。对于后者，假设我们对每一个计算特征值分解 $\hat\ssigma_k=\mathbf U_k\mathbf D_k\mathbf U_k^T$，其中 $\mathbf U_k$ 是 $p\times p$ 的正交矩阵，$\mathbf D_k$ 是正的特征值 $d_{k\ell}$ 组成的对角矩阵。则 $\delta_k(x)$ 式（ 4.12 ） 的组成成分是
...
$$
\underset{a}{\max}\;a^T\mathbf Ba \; \\ \mathrm{st}\; a^T\mathbf Wa=1\tag{4.16}
$$
