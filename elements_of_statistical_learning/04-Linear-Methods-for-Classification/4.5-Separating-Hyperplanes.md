...
\mathrm{\text{subject to}} & y_i(x_i^T\beta+\beta_0)\ge M,\;i=1,\ldots,N
\end{align*}
\tag{4.45}
$$
...
$$
F=\sum\limits_{i=1}^N\alpha_iy_i\tag{4.51}
$$
...
\qquad\qquad \qquad \qquad \qquad \mathrm{\text{subject to}}\; \alpha_i\ge 0\qquad \tag{4.52}
$$
...
从 $式（ 4.50 ）$ 我们可以看到解向量 $\beta$ 定义为**支撑点 (support points)** $x_i$ 的线性组合——这些点通过 $\alpha_i >0$ 定义在 slab 的边界上。图 4.16 显示了我们那个简单例子的最优分离超平面；这里有三个支撑点。同样地，$\beta_0$ 可以通过对任意支撑点求解 $式（ 4.53 ）$ 得到。
